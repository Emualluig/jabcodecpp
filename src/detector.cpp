...
*/
constexpr static jab_boolean checkPatternCross(const std::array<jab_int32, 5>& state_count, jab_float& module_size) noexcept {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
static jab_boolean checkModuleSize3(jab_float size_r, jab_float size_g, jab_float size_b) noexcept {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
static jab_boolean checkModuleSize2(jab_float size1, jab_float size2) noexcept {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean seekPattern(const Bitmap& ch, jab_int32 row, jab_int32 col, jab_int32& start, jab_int32& end, jab_float& center, jab_float& module_size, jab_int32& skip) noexcept {
    jab_int32 state_number = 5;
    jab_int32 cur_state = 0;
    std::array<jab_int32, 5> state_count = {};
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean seekPatternHorizontal(const jab_byte* row, jab_int32& startx, jab_int32& endx, jab_float& centerx, jab_float& module_size, jab_int32& skip) noexcept {
    jab_int32 state_number = 5;
    jab_int32 cur_state = 0;
    std::array<jab_int32, 5> state_count = {};
...
*/
constexpr static jab_int32 crossCheckPatternDiagonal(const Bitmap& image, jab_int32 type, jab_float module_size_max, jab_float& centerx, jab_float& centery, jab_float& module_size, jab_int32& dir, jab_boolean both_dir) noexcept {
    jab_int32 state_number = 5;
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean crossCheckPatternVertical(const Bitmap& image, jab_int32 module_size_max, jab_float centerx, jab_float& centery, jab_float& module_size) noexcept {
	jab_int32 state_number = 5;
	jab_int32 state_middle = (state_number - 1) / 2;
    std::array<jab_int32, 5> state_count = {};
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean crossCheckPatternHorizontal(const Bitmap& image, jab_float module_size_max, jab_float& centerx, jab_float centery, jab_float& module_size) noexcept {
    jab_int32 state_number = 5;
    jab_int32 state_middle = (state_number - 1) / 2;
    std::array<jab_int32, 5> state_count = {};
...
constexpr static jab_boolean crossCheckColor(const Bitmap& image, jab_int32 color, jab_int32 module_size, jab_int32 module_number, jab_int32 centerx, jab_int32 centery) noexcept {
	...
	//vertical
	else if constexpr (direction == Direction::Vertical) {
	...
	//diagonal
	else if constexpr (direction == Direction::Diagonal) {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean crossCheckPatternCh(const Bitmap& ch, jab_int32 type, jab_int32 h_v, jab_float module_size_max, jab_float& module_size, jab_float& centerx, jab_float& centery, jab_int32& dir, jab_int32& dcc) noexcept {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean crossCheckPattern(const std::array<Bitmap, 3>& ch, jab_finder_pattern& fp, jab_int32 h_v) noexcept {
    ...
    if(fp.type == FP1 || fp.type == FP2)
	{
		...
		if(!crossCheckColor<Direction::Horizontal>(ch[2], core_color_in_blue_channel, (jab_int32)fp.module_size, 5, (jab_int32)fp.center.x, (jab_int32)fp.center.y))
			return JAB_FAILURE;
		if(!crossCheckColor<Direction::Vertical>(ch[2], core_color_in_blue_channel, (jab_int32)fp.module_size, 5, (jab_int32)fp.center.x, (jab_int32)fp.center.y))
			return JAB_FAILURE;
		if(!crossCheckColor<Direction::Diagonal>(ch[2], core_color_in_blue_channel, (jab_int32)fp.module_size, 5, (jab_int32)fp.center.x, (jab_int32)fp.center.y))
			return JAB_FAILURE;
	...
	if(fp.type == FP0 || fp.type == FP3)
	{
		...
		if(!crossCheckColor<Direction::Horizontal>(ch[0], core_color_in_red_channel, (jab_int32)fp.module_size, 5, (jab_int32)fp.center.x, (jab_int32)fp.center.y))
			return JAB_FAILURE;
		if(!crossCheckColor<Direction::Vertical>(ch[0], core_color_in_red_channel, (jab_int32)fp.module_size, 5, (jab_int32)fp.center.x, (jab_int32)fp.center.y))
			return JAB_FAILURE;
		if(!crossCheckColor<Direction::Diagonal>(ch[0], core_color_in_red_channel, (jab_int32)fp.module_size, 5, (jab_int32)fp.center.x, (jab_int32)fp.center.y))
			return JAB_FAILURE;
...
*/
constexpr static jab_int32 saveAlignmentPattern(jab_alignment_pattern& ap, std::array<jab_alignment_pattern, MAX_FINDER_PATTERNS>& aps, jab_int32& counter) noexcept {
...
*/
constexpr static void saveFinderPattern(jab_finder_pattern& fp, std::vector<jab_finder_pattern>& fps, jab_int32& counter, std::array<jab_int32, 4>& fp_type_count) noexcept {
...
*/
constexpr static jab_finder_pattern getBestPattern(std::vector<jab_finder_pattern>& fps, jab_int32 fp_count) noexcept {
    jab_int32 counter = 0;
...
*/
constexpr static jab_int32 selectBestPatterns(std::vector<jab_finder_pattern>& fps, jab_int32 fp_count, const std::array<jab_int32, 4>& fp_type_count) noexcept {
    //classify finder patterns into four types
    std::array<std::vector<jab_finder_pattern>, 4> fpsArr = {
        std::vector<jab_finder_pattern>(fp_type_count[FP0]),
        std::vector<jab_finder_pattern>(fp_type_count[FP1]),
        std::vector<jab_finder_pattern>(fp_type_count[FP2]),
        std::vector<jab_finder_pattern>(fp_type_count[FP3])
    };
    std::array<jab_int32, 4> counters = { 0, 0, 0, 0 };
    ...
    for(jab_int32 i = 0; i < fp_count; i++) {
        ...
        jab_int32 index = fps[i].type;
        fpsArr[index][counters[index]++] = fps[i];
    }

    for (jab_int32 i = 0; i < counters.size(); i++) {
        if (counters[i] > 1) {
            fps[i] = getBestPattern(fpsArr[i], counters[i]);
        }
        else if (counters[i] == 1) {
            fps[i] = std::move(fpsArr[i][0]);
        }
        else {
            fps[i] = {};
        }
...
*/
constexpr static void scanPatternVertical(const std::array<Bitmap, 3>& ch, jab_int32 min_module_size, std::vector<jab_finder_pattern>& fps, std::array<jab_int32, 4>& fp_type_count, jab_int32& total_finder_patterns) noexcept {
    jab_boolean done = 0;
        ...
        do
        {
            ...
            if(seekPattern(ch[1], -1, j, starty, endy, centery_g, module_size_g, skip))
            {
                ...
                if(crossCheckPatternVertical(ch[2], module_size_g*2, (jab_float)j, centery_b, module_size_b))
                {
                    ...
                    jab_int32 core_color_in_red_channel = jab_default_palette[FP3_CORE_COLOR * 3 + 0];
                    if(crossCheckColor<Direction::Vertical>(ch[0], core_color_in_red_channel, module_size_r, 5, j, (jab_int32)centery_r))
                    {
                    	type_r = 0;
                ...
                else if(crossCheckPatternVertical(ch[0], module_size_g*2, (jab_float)j, centery_r, module_size_r))
				{
					...
					jab_int32 core_color_in_blue_channel = jab_default_palette[FP2_CORE_COLOR * 3 + 2];
                    if(crossCheckColor<Direction::Vertical>(ch[2], core_color_in_blue_channel, module_size_b, 5, j, (jab_int32)centery_b))
                    {
                    	type_b = 0;
...
*/
constexpr static void seekMissingFinderPattern(const Bitmap& bitmap, std::vector<jab_finder_pattern>& fps, jab_int32 miss_fp_index) noexcept {
    ...
    std::array<Bitmap, 3> rgb = {
        Bitmap(area_width, area_height, 8, 8, 1, area_height * area_width, 0),
        Bitmap(area_width, area_height, 8, 8, 1, area_height * area_width, 0),
        Bitmap(area_width, area_height, 8, 8, 1, area_height * area_width, 0)
    };
    ...
    jab_float pixel_ave[3] = {
        pixel_sum[0] / (jab_float)(area_width * area_height),
        pixel_sum[1] / (jab_float)(area_width * area_height),
        pixel_sum[2] / (jab_float)(area_width * area_height)
    };
    ...
    std::vector<jab_finder_pattern> fps_miss = std::vector<jab_finder_pattern>(MAX_FINDER_PATTERNS,
        { .type = 0, .module_size = 0.0f, .center = { 0, 0 }, .found_count = 0, .direction = 0 }
    );

    jab_int32 total_finder_patterns = 0;
    jab_boolean done = 0;
    std::array<jab_int32, 4> fp_type_count = {};

	for(jab_int32 i=0; i<area_height && done == 0; i++)
    {
        //get row
        const jab_byte* row_r = rgb[0].pixel.data() + i * rgb[0].width;
        const jab_byte* row_g = rgb[1].pixel.data() + i * rgb[1].width;
        const jab_byte* row_b = rgb[2].pixel.data() + i * rgb[2].width;
            ...
            if(seekPatternHorizontal(row_g, startx, endx, centerx_g, module_size_g, skip))
            {
				...
				case 3:
					...
					if(crossCheckPatternHorizontal(rgb[2], module_size_g*2, centerx_b, (jab_float)i, module_size_b))
					{
						...
						module_size_r = module_size_g;
						jab_int32 core_color_in_red_channel = jab_default_palette[FP3_CORE_COLOR * 3 + 0];
						if(crossCheckColor<Direction::Horizontal>(rgb[0], core_color_in_red_channel, module_size_r, 5, (jab_int32)centerx_r, i))
						{
							type_r = 0;
				...
				case 2:
					...
					if(crossCheckPatternHorizontal(rgb[0], module_size_g*2, centerx_r, (jab_float)i, module_size_r))
					{
						...
						module_size_b = module_size_g;
						jab_int32 core_color_in_blue_channel = jab_default_palette[FP2_CORE_COLOR * 3 + 2];
						if(crossCheckColor<Direction::Horizontal>(rgb[2], core_color_in_blue_channel, module_size_b, 5, (jab_int32)centerx_b, i))
						{
							type_b = 0;
							finder_pattern_found = JAB_SUCCESS;
						}
...
*/
constexpr static std::vector<jab_finder_pattern> findMasterSymbol(const Bitmap& bitmap, const std::array<Bitmap, 3>& ch, jab_detect_mode mode, jab_int32& status) noexcept {
    ...
    std::vector<jab_finder_pattern> fps = std::vector<jab_finder_pattern>(MAX_FINDER_PATTERNS, 
        { .type = 0, .module_size = 0.0f, .center = { 0, 0 }, .found_count = 0, .direction = 0 }
    );
    ...
    {
        //get row
        const jab_byte* row_r = ch[0].pixel.data() + i * ch[0].width;
        const jab_byte* row_g = ch[1].pixel.data() + i * ch[1].width;
        const jab_byte* row_b = ch[2].pixel.data() + i * ch[2].width;
        ...
        do
        {
            ...
            if(seekPatternHorizontal(row_g, startx, endx, centerx_g, module_size_g, skip))
            {
                ...
                if(crossCheckPatternHorizontal(ch[2], module_size_g*2, centerx_b, (jab_float)i, module_size_b))
                {
                    ...
                    module_size_r = module_size_g;
                    jab_int32 core_color_in_red_channel = jab_default_palette[FP3_CORE_COLOR * 3 + 0];
                    if(crossCheckColor<Direction::Horizontal>(ch[0], core_color_in_red_channel, module_size_r, 5, (jab_int32)centerx_r, i))
                    {
                    	type_r = 0;
                    	finder_pattern1_found = JAB_SUCCESS;
                    }
                ...
                else if(crossCheckPatternHorizontal(ch[0], module_size_g*2, centerx_r, (jab_float)i, module_size_r))
                {
                    ...
                    module_size_b = module_size_g;
                    jab_int32 core_color_in_blue_channel = jab_default_palette[FP2_CORE_COLOR * 3 + 2];
                    if(crossCheckColor<Direction::Horizontal>(ch[2], core_color_in_blue_channel, module_size_b, 5, (jab_int32)centerx_b, i))
                    {
                    	type_b = 0;
                    	finder_pattern2_found = JAB_SUCCESS;
                    }
    ...
    if(missing_fp_count == 1)
    {
		...
		{
            JAB_REPORT_ERROR("Finder pattern ", miss_fp, " out of image");
...
*/
constexpr static jab_float crossCheckPatternDiagonalAP(const Bitmap& image, jab_int32 ap_type, jab_int32 module_size_max, jab_point center, jab_int32& dir) noexcept {
    jab_int32 offset_x, offset_y;
    jab_boolean fix_dir = 0;
    //if the direction is given, ONLY check the given direction
	if(dir != 0)
...
*/
constexpr static jab_float crossCheckPatternVerticalAP(const Bitmap& image, jab_point center, jab_int32 module_size_max, jab_float& module_size) noexcept {
    jab_int32 state_count[3] = {0};
...
*/
constexpr static jab_float crossCheckPatternHorizontalAP(const jab_byte* row, jab_int32 channel, jab_int32 startx, jab_int32 endx, jab_int32 centerx, jab_int32 ap_type, jab_float module_size_max, jab_float& module_size) noexcept {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean crossCheckPatternAP(const std::array<Bitmap, 3>& ch, jab_int32 y, jab_int32 minx, jab_int32 maxx, jab_int32 cur_x, jab_int32 ap_type, jab_float max_module_size, jab_float& centerx, jab_float& centery, jab_float& module_size, jab_int32& dir) noexcept {
	//get row
	const jab_byte* row_r = ch[0].pixel.data() + y * ch[0].width;
	const jab_byte* row_b = ch[2].pixel.data() + y * ch[2].width;
	...
	if(!crossCheckColor<Direction::Horizontal>(ch[1], core_color_in_green_channel, (jab_int32)module_size, 3, (jab_int32)center.x, (jab_int32)center.y))
		return JAB_FAILURE;
	...
	//again horizontally
	row_r = ch[0].pixel.data() + ((jab_int32)l_centery[0]) * ch[0].width;
	...
	//again horizontally
	row_b = ch[2].pixel.data() + ((jab_int32)l_centery[2]) * ch[2].width;
	...
	if(!crossCheckColor<Direction::Vertical>(ch[1], core_color_in_green_channel, (jab_int32)module_size, 3, (jab_int32)center.x, (jab_int32)center.y))
		return JAB_FAILURE;
	...
	if(!crossCheckColor<Direction::Diagonal>(ch[1], core_color_in_green_channel, (jab_int32)module_size, 3, (jab_int32)center.x, (jab_int32)center.y))
		return JAB_FAILURE;
...
*/
constexpr static jab_alignment_pattern findAlignmentPattern(const std::array<Bitmap, 3>& ch, jab_float x, jab_float y, jab_float module_size, jab_int32 ap_type) noexcept {
    ...
    {
        std::array<jab_alignment_pattern, MAX_FINDER_PATTERNS> aps = {};

        jab_int32 startx = (jab_int32)std::max(0.0f, x - radius);
        jab_int32 starty = (jab_int32)std::max(0.0f, y - radius);
        jab_int32 endx = (jab_int32)std::min(ch[0].width - 1.0f, x + radius);
        jab_int32 endy = (jab_int32)std::min(ch[0].height - 1.0f, y + radius);
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
static jab_boolean findSlaveSymbol(const Bitmap& bitmap, const std::array<Bitmap, 3>& ch, const jab_decoded_symbol& host_symbol, jab_decoded_symbol& slave_symbol, jab_int32 docked_position) noexcept {
    std::array<jab_alignment_pattern, 4> aps = {};
    ...
    switch(docked_position)
    {
        case 3:
            /*
                ...
                fp[3] ... fp[2] .. ap[3] ... ap[2]
            */
            alpha1 = std::atan2(disty01, distx01);
            alpha2 = std::atan2(disty32, distx32);
            sign = 1;
        ...
        case 2:
            /*
                ...
                ap[3] ... ap[2] .. fp[3] ... fp[2]
            */
            alpha1 = std::atan2(disty32, distx32);
            alpha2 = std::atan2(disty01, distx01);
            sign = -1;
            docked_side_size   = slave_symbol.side_size.y;
            undocked_side_size = slave_symbol.side_size.x;
            ap1 = AP2;	//ap[2]
        ...
        case 1:
            /*
                ...
                ap[3] ... ap[2]
            */
            alpha1 = std::atan2(disty12, distx12);
            alpha2 = std::atan2(disty03, distx03);
            sign = 1;
        ...
        case 0:
            /*
                ...
                fp[3] ... fp[2]
            */
            alpha1 = std::atan2(disty03, distx03);
            alpha2 = std::atan2(disty12, distx12);
            sign = -1;
            docked_side_size   = slave_symbol.side_size.x;
            undocked_side_size = slave_symbol.side_size.y;
            ap1 = AP3;	//ap[3]
    ...
    aps[ap1].center.x = host_symbol.pattern_positions[hp1].x + sign * 7 * host_symbol.module_size * std::cos(alpha1);
    aps[ap1].center.y = host_symbol.pattern_positions[hp1].y + sign * 7 * host_symbol.module_size * std::sin(alpha1);
    ...
    {
        JAB_REPORT_ERROR("The first alignment pattern in slave symbol ", slave_symbol.index, " not found");
        return JAB_FAILURE;
    }
    //calculate the coordinate of ap2
    aps[ap2].center.x = host_symbol.pattern_positions[hp2].x + sign * 7 * host_symbol.module_size * std::cos(alpha2);
    aps[ap2].center.y = host_symbol.pattern_positions[hp2].y + sign * 7 * host_symbol.module_size * std::sin(alpha2);
    ...
    {
        JAB_REPORT_ERROR("The second alignment pattern in slave symbol ", slave_symbol.index, "not found");
        return JAB_FAILURE;
    }
    ...
    aps[ap3].center.x = aps[ap1].center.x + sign * (undocked_side_size - 7) * slave_symbol.module_size * std::cos(alpha1);
    aps[ap3].center.y = aps[ap1].center.y + sign * (undocked_side_size - 7) * slave_symbol.module_size * std::sin(alpha1);
    ...
    aps[ap4].center.x = aps[ap2].center.x + sign * (undocked_side_size - 7) * slave_symbol.module_size * std::cos(alpha2);
    aps[ap4].center.y = aps[ap2].center.y + sign * (undocked_side_size - 7) * slave_symbol.module_size * std::sin(alpha2);
    ...
    if(aps[ap3].found_count == 0)
    {
        ...
        {
            JAB_REPORT_ERROR("Alignment pattern ", ap3, " out of image");
			return JAB_FAILURE;
        }
    }
    if(aps[ap4].found_count == 0)
    {
        ...
        {
            JAB_REPORT_ERROR("Alignment pattern ", ap4, " out of image");
			return JAB_FAILURE;
        }
    }
...
*/
constexpr static jab_int32 getSideSize(jab_int32 size, jab_int32& flag) noexcept {
	flag = 1;
...
*/
constexpr static jab_int32 chooseSideSize(jab_int32 size1, jab_int32 flag1, jab_int32 size2, jab_int32 flag2) noexcept {
	if(flag1 == -1 && flag2 == -1)
	{
		return -1;
	}
...
*/
static jab_int32 calculateModuleNumber(const jab_finder_pattern& fp1, const jab_finder_pattern& fp2) noexcept {
	...
	jab_float cos_theta = std::max(fabs(fp2.center.x - fp1.center.x), fabs(fp2.center.y - fp1.center.y)) / dist;
...
*/
static jab_vector2d calculateSideSize(const std::vector<jab_finder_pattern>& fps) noexcept {
...
*/
constexpr static jab_int32 getFirstAPPos(jab_int32 pos) noexcept {
    switch (pos % 3)
    {
    case 0:
        pos--;
...
*/
static jab_int32 detectFirstAP(const std::array<Bitmap, 3>& ch, jab_int32 side_version, const jab_finder_pattern& fp1, const jab_finder_pattern& fp2) noexcept {
    jab_alignment_pattern ap;
     //direction: from FP1 to FP2
    jab_float distx = fp2.center.x - fp1.center.x;
    jab_float disty = fp2.center.y - fp1.center.y;
    jab_float alpha = std::atan2(disty, distx);
    ...
    do
    {
        ...
        ap.center.x = fp1.center.x + distance * std::cos(alpha);
        ap.center.y = fp1.center.y + distance * std::sin(alpha);
...
*/
constexpr static jab_int32 confirmSideVersion(jab_int32 side_version, jab_int32 first_ap_pos) noexcept {
    if(first_ap_pos <= 0)
    {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
static jab_int32 confirmSymbolSize(const std::array<Bitmap, 3>& ch, const std::vector<jab_finder_pattern>& fps, jab_decoded_symbol& symbol) noexcept {
...
*/
static std::optional<Bitmap> sampleSymbolByAlignmentPattern(const Bitmap& bitmap, const std::array<Bitmap, 3>& ch, jab_decoded_symbol& symbol, const std::vector<jab_finder_pattern>& fps) noexcept {
    ...
    std::vector<jab_alignment_pattern> aps = std::vector<jab_alignment_pattern>(number_of_ap_x * number_of_ap_y);
		...
		for(jab_int32 j=0; j<number_of_ap_x; j++)
		{
            ...
            if (i == 0 && j == 0) {
				...
				if(i == 0) {
					...
					aps[index].center.x = aps[j-1].center.x + distance * std::cos(alpha);
					aps[index].center.y = aps[j-1].center.y + distance * std::sin(alpha);
					...
					//calculate the coordinate of ap[index]
					aps[index].center.x = aps[(i-1) * number_of_ap_x].center.x + distance * std::cos(alpha);
					aps[index].center.y = aps[(i-1) * number_of_ap_x].center.y + distance * std::sin(alpha);
    ...
    std::vector<jab_vector2d> rect = std::vector<jab_vector2d>(block_number * 2);
	...
	for(jab_int32 i=0; i<number_of_ap_y-1; i++)
	{
		for(jab_int32 j=0; j<number_of_ap_x-1; j++)
		{
			...
			{
				for(jab_int32 dy=0; dy<= std::min(delta, number_of_ap_y-2) && flag; dy++)
				{
					jab_int32 dx = std::min(delta - dy, number_of_ap_x-2);
					for(jab_int32 dy1=0; dy1<=dy && flag; dy1++)
					{
						...
						for(jab_int32 dx1=0; dx1<=dx && flag; dx1++)
						{
							...
							tl.x = std::max(j - dx1, 0);
							tl.y = std::max(i - dy1, 0);
							br.x = std::min(j + 1 + dx2, number_of_ap_x - 1);
							br.y = std::min(i + 1 + dy2, number_of_ap_y - 1);
    ...
    Bitmap matrix = Bitmap(
        width, height, bitmap.bits_per_channel * bitmap.channel_count, bitmap.bits_per_channel, bitmap.channel_count,
        width * height * mtx_bytes_per_pixel
    );
	...
	for(jab_int32 i=0; i<rect_index; i+=2)
	{
		...
		const jab_perspective_transform pt = perspectiveTransform(
					p0.x, p0.y,
					p1.x, p1.y,
					p2.x, p2.y,
					p3.x, p3.y,
		...
		std::optional<Bitmap> block = sampleSymbol(bitmap, pt, blk_size);
		if(!block.has_value())
		{
...
*/
constexpr static std::array<jab_float, 3> getAveragePixelValue(const Bitmap& bitmap, const std::vector<jab_finder_pattern>& fps) noexcept {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static jab_boolean detectMaster(const Bitmap& bitmap, std::array<Bitmap, 3>& ch, jab_decoded_symbol& master_symbol) noexcept {
    //find master symbol
    jab_int32 status;
    std::vector<jab_finder_pattern> fps = findMasterSymbol(bitmap, ch, jab_detect_mode::INTENSIVE_DETECT, status);
    ...
    else if(status == JAB_FAILURE)
    {
        ...
        std::array<jab_float, 3> rgb_ave = getAveragePixelValue(bitmap, fps);

        //binarize the bitmap using the average pixel values as thresholds
        binarizerRGB(bitmap, ch, rgb_ave);

        //find master symbol
        fps = findMasterSymbol(bitmap, ch, jab_detect_mode::INTENSIVE_DETECT, status);
	...
	const jab_perspective_transform pt = getPerspectiveTransform(fps[0].center, fps[1].center,
															fps[2].center, fps[3].center,
															side_size);

	std::optional<Bitmap> matrix = sampleSymbol(bitmap, pt, side_size);
	...
	jab_int32 decode_result = decodeMaster(matrix.value(), master_symbol);
	...
	else	//if decoding using only finder patterns failed, try decoding using alignment patterns
	{
		...
		std::optional<Bitmap> matrixJ = sampleSymbolByAlignmentPattern(bitmap, ch, master_symbol, fps);
		if(!matrixJ.has_value())
		{
			reportError("Sampling master symbol using alignment pattern failed");
			return JAB_FAILURE;
		}
		decode_result = decodeMaster(matrixJ.value(), master_symbol);
        if (decode_result == JAB_SUCCESS) {
            return JAB_SUCCESS;
        }
        else {
 ...
 *
*/
static std::optional<Bitmap> detectSlave(const Bitmap& bitmap, std::array<Bitmap, 3>& ch, const jab_decoded_symbol& host_symbol, jab_decoded_symbol& slave_symbol, jab_int32 docked_position) noexcept {
    ...
    {
        JAB_REPORT_ERROR("Slave symbol ", slave_symbol.index, " out of image");
        return std::nullopt;
    }

    //calculate perspective transform matrix
    const jab_perspective_transform pt = getPerspectiveTransform(slave_symbol.pattern_positions[0], slave_symbol.pattern_positions[1],
                                                            slave_symbol.pattern_positions[2], slave_symbol.pattern_positions[3],
                                                            slave_symbol.side_size);


    std::optional<Bitmap> matrix = sampleSymbol(bitmap, pt, slave_symbol.side_size);
    if(!matrix.has_value())
    {
        JAB_REPORT_ERROR("Sampling slave symbol ", slave_symbol.index, " failed");
        return std::nullopt;
    }
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
constexpr static bool decodeDockedSlaves(const Bitmap& bitmap, std::array<Bitmap, 3>& ch, std::array<jab_decoded_symbol, MAX_SYMBOL_NUMBER>& symbols, jab_int32 host_index, jab_int32& total) noexcept {
    std::array<jab_int32, 4> docked_positions = {
        symbols[host_index].metadata.docked_position & 0x08,
        symbols[host_index].metadata.docked_position & 0x04,
        symbols[host_index].metadata.docked_position & 0x02,
        symbols[host_index].metadata.docked_position & 0x01,
    };
    ...
    for(jab_int32 j=0; j<4; j++)
    {
        if(docked_positions[j] > 0 && (total)<MAX_SYMBOL_NUMBER)
        {
            ...
            std::optional<Bitmap> matrix = detectSlave(bitmap, ch, symbols[host_index], symbols[total], j);
            if(!matrix.has_value())
            {
                JAB_REPORT_ERROR("Detecting slave symbol ", symbols[total].index, " failed");
                return JAB_FAILURE;
            }
            if(decodeSlave(matrix.value(), symbols[total]) > 0)
            {
                (total)++;
            }
            else
...
*/
static std::optional<std::vector<jab_byte>> decodeJABCodeEx(Bitmap& bitmap, jab_int32 mode, jab_int32& status, std::array<jab_decoded_symbol, MAX_SYMBOL_NUMBER>& symbols, jab_int32 max_symbol_number) noexcept {
	status = 0;
    ...
    std::array<Bitmap, 3> ch = binarizerRGB(bitmap); // Initialized in binarizerRGB
    ...
    {
        total_data_length += symbols[i].data.size();
    }
    std::vector<jab_char> decoded_bits = std::vector<jab_char>(total_data_length);
    jab_int32 offset = 0;
    for(jab_int32 i=0; i<total; i++)
    {
        jab_char* dst = decoded_bits.data();
        dst += offset;
        memcpy(dst, symbols[i].data.data(), symbols[i].data.size());
        offset += symbols[i].data.size();
    }

    //decode data
    std::optional<std::vector<jab_byte>> decoded_data = decodeData(decoded_bits);
 ...
 * @return the decoded data | NULL if failed
*/
std::optional<std::vector<jab_byte>> decodeJABCode(Bitmap& bitmap, jab_int32 mode, jab_int32& status) noexcept {
    std::array<jab_decoded_symbol, MAX_SYMBOL_NUMBER> symbols = {};
