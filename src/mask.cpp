...
*/
constexpr static jab_int32 applyRule1(const std::vector<jab_int32>& matrix, jab_int32 width, jab_int32 height, jab_int32 color_number) noexcept {
 ...
 * @return the penalty score
*/
constexpr static jab_int32 applyRule2(const std::vector<jab_int32>& matrix, jab_int32 width, jab_int32 height) noexcept {
	jab_int32 score = 0;
 ...
 * @return the penalty score
*/
constexpr static jab_int32 applyRule3(const std::vector<jab_int32>& matrix, jab_int32 width, jab_int32 height) noexcept {
	jab_int32 score = 0;
 ...
 * @return the penalty score
*/
constexpr static jab_int32 evaluateMask(const std::vector<jab_int32>& matrix, jab_int32 width, jab_int32 height, jab_int32 color_number) noexcept {
...
*/
jab_int32 maskCode(Encode& enc, const CodeParams& cp) noexcept {
	jab_int32 mask_type = 0;
	jab_int32 min_penalty_score = 10000;

	std::vector<jab_int32> masked = std::vector<jab_int32>(cp.code_size.x * cp.code_size.y, -1);
...
void demaskSymbol(std::vector<jab_char>& data, const std::vector<jab_byte>& data_map, jab_vector2d symbol_size, jab_int32 mask_type, jab_int32 color_number) noexcept {
	...
	for (jab_int32 x = 0; x < symbol_width; x++)
	{
		for (jab_int32 y = 0; y < symbol_height; y++)
		{
			...
			{
				if (count > data.size() - 1) return;
				jab_int32 index = data[count];
				switch (mask_type)
				{
				case 0:
